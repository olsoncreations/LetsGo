...
const STORAGE_KEY = "letsgo-apply-sales-rep";
...
export default function SalesRepApplicationPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const res = await fetch("/api/applications?type=sales_rep", {
      ...
      // Load commission rates from sales_config
      try {
        const ratesRes = await fetch("/api/sales-config");
        if (ratesRes.ok) {
  ...
  const handleSubmit = async () => {
    ...
    if (dlFile) {
      ...
      const path = `sales-rep-applications/${session.user.id}/drivers-license-${Date.now()}.${ext}`;
    ...
    if (resumeFile) {
      ...
      const path = `sales-rep-applications/${session.user.id}/resume-${Date.now()}.${ext}`;
      const { error: uploadErr } = await supabaseBrowser.storage
        .from("documents")
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #000 0%, #0a0a1a 50%, #0f0f1f 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: 24, height: 24, border: `2px solid ${NEON.primary}`, borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
      </div>
    );
  }
      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #000 0%, #0a0a1a 50%, #0f0f1f 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
        <div style={{ maxWidth: 480, width: "100%", textAlign: "center", padding: "48px 32px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: `1px solid rgba(${NEON.yellowRGB},0.2)` }}>
          ...
          <p style={{ fontSize: 14, color: "rgba(255,255,255,0.5)", lineHeight: 1.6, marginBottom: 8 }}>
            You submitted a sales rep application on {new Date(existingApp.created_at).toLocaleDateString()}.
          </p>
          ...
          <div style={{ ...sectionStyle }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer" }}>
                ...
                />
                <span style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", lineHeight: 1.6 }}>
                  I agree to the <span onClick={e => { e.preventDefault(); router.push("/terms"); }} style={{ color: NEON.primary, cursor: "pointer", textDecoration: "underline" }}>Terms of Service</span> and understand that my application will be reviewed by the LetsGo team. *
                </span>
              </label>
            </div>
          </div>
