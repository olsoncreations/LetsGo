    ...
    <div style={{position:"fixed",inset:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.85)",backdropFilter:"blur(12px)",animation:"fadeIn 0.3s ease"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:420,borderRadius:6,background:"#0C0C14",border:`1px solid rgba(${mode==="confirm"?NEON.yellowRGB:NEON.pinkRGB},0.2)`,boxShadow:`0 0 60px rgba(${mode==="confirm"?NEON.yellowRGB:NEON.pinkRGB},0.08)`,padding:28,textAlign:"center"}}>
        {mode==="confirm"?(<>
          ...
          <p style={{fontSize:12,color:"rgba(255,255,255,0.35)",marginBottom:24,lineHeight:1.6,maxWidth:320,margin:"0 auto 24px"}}>Connect your Venmo in Settings to receive earnings.</p>
          <div style={{display:"flex",gap:10,justifyContent:"center"}}>
...
const SettingsModal = ({open,onClose,profile,avatarUrl,onAvatarChange,onProfileSaved,token}:{
  ...
  const [venmoUsername,setVenmoUsername]=useState(profile?.payout_identifier||"");
  ...
  useEffect(()=>{
    if(!open||!token||activeTab!=="privacy"||!profile?.id)return;
  ...
  const handleConnectVenmo=async()=>{if(!token||!venmoUsername.trim())return;setSaving(true);const res=await fetch("/api/users/profile",{method:"PATCH",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({payout_method:"venmo",payout_identifier:venmoUsername.trim()})});if(res.ok){const d=await res.json();onProfileSaved(d.profile);}setSaving(false);};
  const handleDisconnectVenmo=async()=>{if(!token)return;setSaving(true);const res=await fetch("/api/users/profile",{method:"PATCH",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({payout_method:"",payout_identifier:""})});if(res.ok){const d=await res.json();onProfileSaved(d.profile);setVenmoUsername("");}setSaving(false);};
  ...
  const venmoConnected=!!(profile?.payout_method&&profile?.payout_identifier);
  if(!open)return null;
  return(
    <div style={{position:"fixed",inset:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.85)",backdropFilter:"blur(12px)",animation:"fadeIn 0.3s ease"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:640,maxHeight:"90vh",borderRadius:6,background:"#0C0C14",border:`1px solid rgba(${NEON.primaryRGB},0.2)`,boxShadow:`0 0 60px rgba(${NEON.primaryRGB},0.08)`,overflow:"auto"}}>
        ...
        <div style={{padding:24}}>
          ...
          {activeTab==="privacy"&&(<div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          {activeTab==="account"&&(<div style={{display:"flex",flexDirection:"column",gap:16}}>
            <div style={{padding:16,borderRadius:4,background:venmoConnected?"rgba(255,255,255,0.02)":`rgba(${NEON.yellowRGB},0.03)`,border:`1px solid ${venmoConnected?"rgba(255,255,255,0.06)":`rgba(${NEON.yellowRGB},0.15)`}`}}>
...
export default function LetsGoProfile() {
  ...
  const profileTourSteps: TourStep[] = useMemo(() => [
    ...
    { target: '[data-tour="cashout-btn"]', title: "Cash Out Anytime", description: "Once you hit the minimum, you can cash out directly to Venmo. Your money, your way.", position: "top" },
    ...
    { target: '[data-tour="settings-btn"]', title: "Connect Your Payment", description: "Don't forget to link your Venmo in Settings so you can cash out your earnings. You'll find it under the Account tab.", position: "bottom" },
  ], []);
    ...
    (async () => {
      ...
      if (cashoutRows) {
        setCashouts(cashoutRows.map((c: Record<string, unknown>) => ({
          ...
          method: (c.method as string) || "Venmo",
            ...
            <NeonBorderCard neon={NEON.green} neonRGB={NEON.greenRGB}>
              <div className="earnings-banner" style={{ padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div className="earnings-values" style={{ display: "flex", gap: 40, alignItems: "center" }}>
                  ...
                  <div className="earnings-divider" style={{ width: 1, height: 40, background: "rgba(255,255,255,0.06)" }} />
                  <button data-tour="cashout-btn" onClick={() => setCashOutModal(venmoConnected ? "confirm" : "noVenmo")} style={{ padding: "10px 22px", borderRadius: 3, border: `1px solid rgba(${NEON.yellowRGB},0.5)`, background: `rgba(${NEON.yellowRGB},0.1)`, color: NEON.yellow, fontSize: 11, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", textShadow: `0 0 8px ${NEON.yellow}50` }}>Cash Out</button>
                </div>
                        ...
                        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 16 }}>
                          ...
                          {biz.balance >= minCashoutCents / 100 && (
                            <div style={{ marginTop: 14, padding: "10px 16px", borderRadius: 4, background: `rgba(${NEON.yellowRGB},0.06)`, border: `1px solid rgba(${NEON.yellowRGB},0.15)`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                              ...
                              <button onClick={(e) => { e.stopPropagation(); setCashOutModal(venmoConnected ? "confirm" : "noVenmo"); }} style={{ padding: "5px 14px", borderRadius: 3, border: `1px solid rgba(${NEON.yellowRGB},0.4)`, background: `rgba(${NEON.yellowRGB},0.1)`, color: NEON.yellow, fontSize: 9, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Cash Out</button>
                            </div>
                          )}
